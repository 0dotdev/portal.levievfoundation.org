...
class ApplicationExporter extends Exporter
{
    ...
    public static function getColumns(): array
    {
        return [
            ...
            ExportColumn::make('father_first_name')->label('Father Name')->formatStateUsing(fn($record) => $record->father_first_name  . ' ' . $record->father_last_name),
            ExportColumn::make('father_phone')->label('Father Phone'),
            ExportColumn::make('father_email')->label('Father Email'),
            ExportColumn::make('mother_first_name')->label('Mother Name')->formatStateUsing(fn($record) => $record->mother_first_name  . ' ' . $record->mother_last_name),
            ExportColumn::make('mother_phone')->label('Mother Phone'),
            ExportColumn::make('mother_email')->label('Mother Email'),
            ExportColumn::make('father_address')->label('Home Address')->formatStateUsing(fn($record) => $record->father_address . ', ' . $record->father_city . ', ' . $record->father_state . ' ' . $record->father_zip),
            ExportColumn::make('mother_address')->label('Mother Address')->formatStateUsing(fn($record) => $record->mother_address . ', ' . $record->mother_city . ', ' . $record->mother_state . ' ' . $record->mother_zip),
